...
class DataGetter:
    ...
    def __init__(self, which='train', batch_size=128, component='skymap',
        ...
        self.result_dir = os.path.join(self.map_dir, "%s" % (component))
    ...
    def next_batch(self):
        ...
        for j in range(self.batch_size):
            idx = self.labels.index[self.i]
            curr_img = np.load(os.path.join(self.result_dir, "%d.npy"%idx))
            ...
            #labels[j, self.labels.loc[idx, 'y']] = 1
            #labels[j] = self.labels.loc[idx, 'y']
            labels[j] = self.labels.iloc[self.i]['y']
            self.i = self.i + 1
            if self.i == self.n: self.i = 0
            #ipdb.set_trace()
        img = np.stack(curr_batch, axis=0).astype(np.float32)
...
class DEEPSZ(object):
    ...
    def __getitem__(self, i):
        ...
        labels = np.zeros([self.batch_size, 2])
        for j in range(self.batch_size):
            idx = self.labels.index[(j + self.batch_size * i)%self.n]
            curr_img = np.load(os.path.join(self.map_component_dir, "%d.npy"%idx))
            if self.normalize:
                _min, _max = curr_img.min(), curr_img.max()
                curr_img = (curr_img - _min) / (_max - _min)
            curr_batch.append(curr_img)
            labels[j, 1 if self.labels.loc[idx, 'y'] else 0] = 1
        ...
        if i >= self.n: raise IndexError("End")
        ...
        curr_img = torch.tensor(curr_img)

        targets = torch.tensor([0,1]) if self.labels.loc[idx,'y'] else torch.tensor([1,0])
        return curr_img, targets, i
        """
